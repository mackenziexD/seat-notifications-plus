...
class StructureUnanchoring extends AbstractNotification
{
    ...
    public function toSlack($notifiable)
    {
        return (new SlackMessage)
            ...
            ->title('Structure Unanchoring Started')
            ->thumb('https://images.evetech.net/types/'.$type->typeID.'/icon?size=128')
            ->content("The {$type->typeName} **{$structureName}** has started unanchoring in {$this->zKillBoardToDiscordLink('system',$system->itemID,$system->itemName)} ({$region}). It will be unanchored at **{$timeLeft}**.")
