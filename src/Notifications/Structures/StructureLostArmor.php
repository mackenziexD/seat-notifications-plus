...
class StructureLostArmor extends AbstractDiscordNotification
{
    ...
    public function populateMessage(DiscordMessage $message, $notifiable)
    {
        $message->embed(function (DiscordEmbed $embed) {
            ...
            $embed->title('Structure Lost Armor');
            $embed->description("The {$type->typeName} **{$structureName}** in {$this->zKillBoardToDiscordLink('system',$system->itemID,$system->itemName)} ({$region}) has lost its shields. Hull timer end at: **{$timeLeft}**.");
